 ...
 * @param lat 纬度
 */
function transBMapToTMap(lng: number, lat: number) {
 ...
 * @param lat 纬度
 */
function transTMapToBMap(lng: number, lat: number) {
  let B_PI = 3.14159265358979324 * 3000.0 / 180.0;
  let x = lng;
  let y = lat;
  let z = Math.sqrt(x * x + y * y) + 0.00002 * Math.sin(y * B_PI);
 ...
 */
function verifyParams(params: CoordinateParams) {
 ...
 */
function throwError(errText: string) {
 ...
 * @example
 ...
 ```
 */
function transCoordinate(params: CoordinateParams) {
  const { from, to, lng, lat } = params
  let result = {
    lat: 0,
    lng: 0
